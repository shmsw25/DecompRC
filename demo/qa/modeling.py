...
class BertForQuestionAnswering(nn.Module):
    ...
    def __init__(self, config, n_class, variant_id=0):
        super(BertForQuestionAnswering, self).__init__()
        self.bert = BertModel(config)
        self.qa_outputs = nn.Linear(config.hidden_size, 2) # [N, L, H] => [N, L, 2]
        self.qa_classifier = nn.Linear(config.hidden_size, n_class) # [N, H] => [N, 3]
    ...
    def forward(self, batch, global_step=0):
        ...
        if len(batch) == 7:
            #start_positions, end_positions, switch = batch[3:]
            ...
            span_mask = answer_mask * (switch == 0).type(torch.FloatTensor).cuda()
            ...
            if self.variant_id == 1:
                # entropy add
                span_loss = torch.stack(start_losses)+torch.stack(end_losses)
                entropy_span = - span_loss * torch.log(span_loss + 1e-4)
                total_loss += (global_step/10000.0) * torch.sum(entropy_span)
...
class BertForQuestionAnsweringWithKeyword(nn.Module):

    def __init__(self, config, n_class, variant_id=0, output_dropout_prob=0):
        super(BertForQuestionAnsweringWithKeyword, self).__init__()
        self.bert = BertModel(config)
        self.qa_outputs = nn.Linear(config.hidden_size, 3) # [N, L, H] => [N, L, 2]
        self.qa_classifier = nn.Linear(config.hidden_size, n_class) # [N, H] => [N, 3]
    ...
    def forward(self, batch, global_step=0):
        ...
        start_logits, end_logits, keyword_logits = logits.split(1, dim=-1)
        start_logits = start_logits.squeeze(-1)
        end_logits = end_logits.squeeze(-1)
        keyword_logits = keyword_logits.squeeze(-1)

        switch_logits = self.qa_classifier(torch.max(sequence_output, 1)[0])

        if len(batch) == 8:
            #start_positions, end_positions, switch = batch[3:]
            start_positions, end_positions, keyword_positions,  switch, answer_mask = batch[3:]
            ignored_index = start_logits.size(1)
            start_positions.clamp_(0, ignored_index)
            end_positions.clamp_(0, ignored_index)
            keyword_positions.clamp_(0, ignored_index)
            answer_mask = answer_mask.type(torch.FloatTensor).cuda()
            loss_fct = CrossEntropyLoss(ignore_index=ignored_index, reduce=False)
            span_mask = answer_mask * (switch == 0).type(torch.FloatTensor).cuda()
            ...
            end_losses = [(loss_fct(end_logits, _end_positions) * _span_mask) \
                            for (_end_positions, _span_mask) \
                          in zip(torch.unbind(end_positions, dim=1), torch.unbind(span_mask, dim=1))]
            keyword_losses = [(loss_fct(keyword_logits, _keyword_positions) * _span_mask) \
                            for (_keyword_positions, _span_mask) \
                          in zip(torch.unbind(keyword_positions, dim=1), torch.unbind(span_mask, dim=1))]
            switch_losses = [(loss_fct(switch_logits, _switch) * _answer_mask) \
                            for (_switch, _answer_mask) \
                            in zip(torch.unbind(switch, dim=1), torch.unbind(answer_mask, dim=1))]
            # start_losses = [N, 30]
            # end_losses = [N, 30]
            # joint_losses = [N, 30]
            assert len(start_losses) == len(end_losses) == len(keyword_losses) == len(switch_losses)
            return torch.sum(sum(start_losses + end_losses + keyword_losses +  switch_losses))

        elif len(batch) == 3:
            return start_logits, end_logits, keyword_logits, switch_logits
